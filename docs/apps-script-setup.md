...
Os scripts prontos para copiar ficam aqui:

- [`docs/apps-script/email-snapshot-personal-template.gs`](/Users/tharsovieira/Documents/DailyLife/Prumo/docs/apps-script/email-snapshot-personal-template.gs)
- [`docs/apps-script/email-snapshot-work-template.gs`](/Users/tharsovieira/Documents/DailyLife/Prumo/docs/apps-script/email-snapshot-work-template.gs)

Os dois arquivos têm a mesma estrutura. A única diferença real é `CONFIG.expectedAccount`. Isso é deliberado: se você colar o script da conta errada, ele falha cedo, em vez de gerar snapshot no lugar errado e te dar uma falsa sensação de controle. Mentira elegante continua sendo mentira.
...
O JSON gerado segue a issue #30:
...
5. Cole o conteúdo de [`docs/apps-script/email-snapshot-personal-template.gs`](/Users/tharsovieira/Documents/DailyLife/Prumo/docs/apps-script/email-snapshot-personal-template.gs) em `Code.gs`.
...
1. Saia da conta errada antes de começar. Apps Script com multi-login é o tipo de pegadinha que parece piada ruim, mas cobra caro.
...
6. Cole o conteúdo de [`docs/apps-script/email-snapshot-work-template.gs`](/Users/tharsovieira/Documents/DailyLife/Prumo/docs/apps-script/email-snapshot-work-template.gs) em `Code.gs`.
...
Se o briefing do Prumo roda autenticado só na conta pessoal do Google Drive, a snapshot da conta de trabalho não vai ficar visível por telepatia. Você precisa criar a ponte:
...
Sem esse compartilhamento, o Apps Script da conta de trabalho funciona, mas o briefing continua cego de um olho.

## Escopos e permissões

Os escopos exatos podem variar ligeiramente porque Apps Script gosta de abstração demais, mas estes são os acessos esperados:
...
Na prática, o consent screen vai pedir algo nessa linha. Não tem web app, não tem endpoint público, não tem token manual. Só o velho pacto faustiano padrão do Apps Script.
...
- Se o Drive falhar, a execução falha de verdade. Aí não tem mágica. Sem arquivo não existe snapshot, só uma desculpa bem formatada
...
- O arquivo é sobrescrito no mesmo path a cada execução. Isso é intencional. Snapshot não é museu
- Para esse caso, você não precisa usar `Deploy > New deployment`. Isso só serviria para criar superfície desnecessária
