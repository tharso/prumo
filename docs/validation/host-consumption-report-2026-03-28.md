...
| Responde a degradação sem melodrama nem negação | Sim | `ok` → seguiu em frente sem drama | PASS |
...
O runtime detectou workspace legado e exigiu `migrate` antes de aceitar `start`. O host (Claude Code) respondeu a isso de forma correta: rodou migrate com os parâmetros certos sem inventar fluxo alternativo. Porém, isso adiciona um passo que não está coberto nos 4 cenários do protocolo de validação.

**Sugestão:** Considerar um Cenário 5 no protocolo — "Workspace legado sem identidade canônica" — para validar que o host não tenta `start` nem `briefing` antes de `migrate`.

### 8.2 Integrações desconectadas

Google e Apple Reminders estão ambos `disconnected`. O host tratou isso corretamente como estado operacional e não elevou a drama central. A ação `auth-google` aparece na lista de `actions[]` com prioridade 50, corretamente abaixo de briefing (90), continue (80) e process-inbox (70).

### 8.3 Frente quente disponível mas não priorizada

`state_flags.has_continue_item = true` e `actions[1]` contém o item de continuação (abertura ME/SLU + plano PME). O runtime decidiu corretamente que briefing tem precedência sobre continuação quando `has_briefed_today = false`. O host respeitou essa decisão sem tentar inverter.
