...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--handover-max-bytes", type=int, default=120_000)
    p.add_argument("--handover-max-lines", type=int, default=350)
    p.add_argument("--handover-keep-closed", type=int, default=8)

    p.add_argument("--inbox-min-total", type=int, default=8)
    p.add_argument("--inbox-min-media", type=int, default=4)
    return p.parse_args()
...
def main() -> int:
    ...
    last_apply_at = parse_iso(state.get("last_apply_at"))
    cooldown_ok = True
    ...
    handover_over_limit = handover_size >= args.handover_max_bytes or handover_lines >= args.handover_max_lines
    handover_summary_missing = not handover_summary_path.exists()
    handover_trigger = handover_exists and (
        (handover_over_limit and handover_closed > args.handover_keep_closed)
    ...
    inbox_trigger = bool(inbox_files) and (
        inbox_total >= args.inbox_min_total
        or inbox_media >= args.inbox_min_media
        or preview_stale
    )
    ...
    actions: list[dict[str, str | int | bool]] = []
    ...
    state_payload = {
        "last_run_at": now_iso(),
        "last_apply_at": now_iso() if should_apply else state.get("last_apply_at", ""),
        ...
        "thresholds": {
            "handover_max_bytes": args.handover_max_bytes,
            "handover_max_lines": args.handover_max_lines,
            "handover_keep_closed": args.handover_keep_closed,
            "inbox_min_total": args.inbox_min_total,
            "inbox_min_media": args.inbox_min_media,
        },
        "decision": {
