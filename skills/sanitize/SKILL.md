...
1. Executar:
   - `if [ -f scripts/prumo_sanitize_state.py ]; then python3 scripts/prumo_sanitize_state.py --workspace .; elif [ -f scripts/prumo_sanitize_state.py ]; then python3 scripts/prumo_sanitize_state.py --workspace .; else python3 Prumo/scripts/prumo_sanitize_state.py --workspace .; fi`
   - opcional (gatilhos automáticos + calibração por workspace): `if [ -f scripts/prumo_auto_sanitize.py ]; then python3 scripts/prumo_auto_sanitize.py --workspace .; elif [ -f scripts/prumo_auto_sanitize.py ]; then python3 scripts/prumo_auto_sanitize.py --workspace .; else python3 Prumo/scripts/prumo_auto_sanitize.py --workspace .; fi`
...
1. Se o usuário confirmar, executar:
   ...
   - opcional (automático): `if [ -f scripts/prumo_auto_sanitize.py ]; then python3 scripts/prumo_auto_sanitize.py --workspace . --apply; elif [ -f scripts/prumo_auto_sanitize.py ]; then python3 scripts/prumo_auto_sanitize.py --workspace . --apply; else python3 Prumo/scripts/prumo_auto_sanitize.py --workspace . --apply; fi`
