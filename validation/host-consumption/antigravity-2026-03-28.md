...
**Objetivo:** Validar o contrato estruturado implementado no runtime (Issue `#61`) e atestar se o host adapter já detém capacidade de navegação confiável puramente via payload JSON de `prumo start` e `prumo briefing`, extinguindo o paradigma inseguro de "pescar prosa" e evitando tool-chaining narrativo.
...
**Diagnóstico geral:** A infraestrutura e as tipagens dessas chaves estão sólidas; os booleanos e indicadores já eliminam totalmente o risco de regressão para extração cega e contorcionismos textuais. O adapter não inspeciona mais as raízes geradas para o leitor final.
...
- **Condição/Sinal:** `next_move.id = "briefing"`, chancelado pelo `selection_contract`.
- **Ação do host:** A proposta é convertida usando confirmações textuais breves de aceite (`"1"`, `"ok"`). A ação correlacionada ao índice explícito é executada sem renderização de menus pendentes ou devaneio conversacional.
...
- **Ação do host:** Prioriza sumariamente a pauta quente ou pendente, desprezando sugestões genéricas decorativas que outrora seriam pescadas em `message`.
...
- **Ação do host:** Operação baseada primariamente nos dados sadios, preservando a utilidade do que está funcional sem reações exaustivamente dramáticas sobre os pedaços caídos.
- **Resultado:** PASS. Resiliência de automação frente a falhas limitadas, sem sobrepor deficiências periféricas ao andamento útil.
